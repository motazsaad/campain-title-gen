...
# in feed file will take 3 random keyword as seed to generate new title
...
out_csv_file = "trainHistoryDict/titleComparison3words.csv"
...
print("\"", seed, "\"")
# pick 3 random keywords from title if the title greater than 3 words
# make this 3 words as seed to generate new title
num_rand_words = 3
seedList = seed.split()
if len(seedList) > num_rand_words:
    newseed = random.sample(seedList, num_rand_words)
    newseed = ' '.join(newseed)
else:
    newseed = seed
print("New Seed (3 random keywords):")
print("\"", newseed, "\"")

pattern = [char_to_int[value] for value in newseed]
print('New Pattern: {} '.format(pattern))
print('Num New Pattern: {} '.format(len(pattern)))
...
print("original_title, 3_rand_words, generated _text, original_desc and perplexity PPL for whole text to csv file")

list_row = [originalTitle,newseed, generatedText,originalDesc,ppl_val]
dfOriginal = pd.read_csv(out_csv_file, index_col=False)
dfNew = pd.DataFrame([list_row],
                   columns=['original_title','3_rand_words','generated _text','original_desc','perplexity'])
